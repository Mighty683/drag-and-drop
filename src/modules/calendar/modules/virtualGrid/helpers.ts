    ...
    for (const processedEvent of nodeEvents) {
        if (!gridWidthX) {
            ...
            for (let rowIndex = 0; rowIndex < gridHeightY; rowIndex++) {
                ...
                if (
                    rowIndex === 0 ||
                    newEventPositionX <= bestFittedEventPositionX
                ) {
...
export function isEventFinishedBeforeSnappedToGrid(
    ...
    return (
        eventAEndSnappedToGrid.getTime() <= eventBStartSnappedToGrid.getTime()
    );
}
...
export function areEventsOverlappingInGrid(
    ...
    return (
        (eventAStartSnappedToGrid.getTime() >=
            eventBStartSnappedToGrid.getTime() &&
            eventAStartSnappedToGrid.getTime() <
                eventBEndSnappedToGrid.getTime()) ||
        (eventBStartSnappedToGrid.getTime() >=
            eventAStartSnappedToGrid.getTime() &&
            eventBStartSnappedToGrid.getTime() <
                eventAEndSnappedToGrid.getTime())
    );
}
